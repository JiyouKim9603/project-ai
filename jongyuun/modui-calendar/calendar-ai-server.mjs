...
const getOllamaErrorMessage = (error) => {
  ...
  if (
    ...
    return [
      ...
      'Ollama가 실행 중인지 확인하고, 터미널에서 `ollama list` 또는 `ollama run gemma3`를 실행해주세요.',
    ].join(' ')
  }
  ...
  try {
    ...
    if (!ollamaResponse.ok) {
      const apiMessage = typeof data?.error === 'string'
        ? data.error
        : `Ollama API 오류 (${ollamaResponse.status})`
      res.statusCode = ollamaResponse.status >= 500 ? 502 : 400
      res.end(JSON.stringify({ error: apiMessage }))
      return
    }

    const rawReply = typeof data?.message?.content === 'string' ? data.message.content.trim() : ''
    if (!rawReply) {
      res.statusCode = 502
      res.end(JSON.stringify({ error: 'Ollama 응답에서 텍스트를 찾지 못했습니다.' }))
      return
    }
    ...
    try {
      ...
      res.statusCode = 502
      res.end(JSON.stringify({
        error: 'Ollama가 일정 작업 형식을 올바르게 반환하지 못했습니다. 같은 요청을 다시 시도해주세요.',
      }))
      return
    }
    ...
    res.statusCode = 500
    res.end(JSON.stringify({ error: getOllamaErrorMessage(error) }))
  }
}
